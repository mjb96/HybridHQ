...
// ==========================================
// HYBRID INTEL ENGINE: CONTEXTUAL REASONING
// ==========================================
export function computeHybridInsights(state) {
  const insights = [];
  if (!state || !state.weeks || !state.currentWeek) return insights;

  const currentWkNum = parseInt(state.currentWeek, 10);
  const crossMap = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
  const todayKey = crossMap[new Date().getDay()];
  
  // Find Yesterday's Context Key
  const yesterdayIdx = (new Date().getDay() + 6) % 7;
  const yesterdayKey = crossMap[yesterdayIdx];
  const yesterdayWkKey = yesterdayKey === 'sun' && currentWkNum > 1 ? String(currentWkNum - 1) : String(currentWkNum);

  // 1. Cross-Domain Interference Check
  try {
    const yesterdayGymStats = state.weeks[yesterdayWkKey]?.gymStats?.[yesterdayKey] || {};
    const yesterdayRunStats = state.weeks[yesterdayWkKey]?.runs?.[yesterdayKey] || {};
    const yesterdayAnaerobicTE = parseFloat(yesterdayGymStats.anaerobicTE || yesterdayGymStats.trainingEffect || 0);
    const yesterdayRunTE = parseFloat(yesterdayRunStats.trainingEffect || 0);
    
    const todaysLifts = state.weeks[String(currentWkNum)]?.lifts?.[todayKey] || {};
    const hasHeavyLiftsToday = Object.keys(todaysLifts).some(l => 
      l.toLowerCase().includes('squat') || l.toLowerCase().includes('deadlift') || l.toLowerCase().includes('press')
    );

    if (hasHeavyLiftsToday && (yesterdayAnaerobicTE >= 4.0 || yesterdayRunTE >= 4.0)) {
      insights.push({
        id: 'interference_' + todayKey,
        type: 'warning',
        title: '💥 CNS Fatigue Advisory',
        text: `You logged a significant training effect (${Math.max(yesterdayAnaerobicTE, yesterdayRunTE).toFixed(1)}) yesterday. Central nervous system strain might limit your absolute power on today's compound lifts. Consider capping working sets at RPE 8.`
      });
    }
  } catch (e) { console.warn("Interference engine error:", e); }

  // 2. Aerobic Efficiency Gains
  try {
    if (currentWkNum >= 3) {
      let recentHRs = [], olderHRs = [];
      for (let w = currentWkNum; w >= Math.max(1, currentWkNum - 3); w--) {
        const wData = state.weeks[String(w)];
        if (!wData?.runs) continue;
        for (const dk of crossMap) {
          const run = wData.runs[dk];
          const hr = parseFloat(run?.avgHR || 0);
          const dist = parseFloat(run?.dist || 0);
          if (hr > 100 && dist > 2) {
            if (w >= currentWkNum - 1) recentHRs.push(hr);
            else olderHRs.push(hr);
          }
        }
      }
      if (recentHRs.length && olderHRs.length) {
        const avgRecent = recentHRs.reduce((a, b) => a + b, 0) / recentHRs.length;
        const avgOlder = olderHRs.reduce((a, b) => a + b, 0) / olderHRs.length;
        if (avgRecent < avgOlder - 3) {
          insights.push({
            id: 'efficiency_gain',
            type: 'success',
            title: '🏆 Aerobic Base Expanding',
            text: `Your mean cardiac cost during steady-state runs has fallen by ${Math.round(avgOlder - avgRecent)} BPM compared to earlier baseline blocks. Your cardiac adaptation is functioning—you are cultivating real efficiency.`
          });
        }
      }
    }
  } catch (e) { console.warn("Efficiency engine error:", e); }

  // 3. Strength-to-Weight Optimization
  try {
    const stats = state.exerciseStats || {};
    let topLift = null, maxPR = 0;
    ['Back Squat', 'Bench Press', 'Deadlift'].forEach(l => {
      if (stats[l]?.allTimeMax > maxPR) { maxPR = stats[l].allTimeMax; topLift = l; }
    });

    if (topLift && Array.isArray(state.bodyWeightLog) && state.bodyWeightLog.length >= 2) {
      const sortedLog = [...state.bodyWeightLog].sort((a, b) => new Date(b.date) - new Date(a.date));
      const latestWeight = sortedLog[0].weight;
      const olderWeight = sortedLog[sortedLog.length - 1].weight;
      
      if (latestWeight < olderWeight - 1.0) {
        insights.push({
          id: 'strength_to_weight',
          type: 'info',
          title: '⚖️ Relative Power Optimization',
          text: `Your scale mass has trended down by ${(olderWeight - latestWeight).toFixed(1)}kg while protecting your absolute estimated 1RM ceiling on ${topLift}. Your strength-to-weight profile is systematically improving.`
        });
      }
    }
  } catch (e) { console.warn("Relative power engine error:", e); }

  // 4. ACWR Danger Zone Check
  try {
    const lift = [], run = [];
    for (let w = 1; w <= currentWkNum; w++) {
      const wkData = state.weeks[String(w)];
      let gymLoad = 0, runLoad = 0;
      if (wkData) {
        crossMap.forEach(d => {
          const gRpe = parseInt(wkData.gymRpe?.[d], 10) || 0;
          const gTime = wkData.gymStats?.[d]?.time || '0';
          const parts = gTime.split(':').map(Number);
          const gMin = parts.length === 2 ? parts[0] + parts[1]/60 : parseInt(gTime, 10) || 0;
          if (gRpe > 0 && gMin > 0) gymLoad += gRpe * gMin;

          const rRpe = parseInt(wkData.runs?.[d]?.rpe, 10) || 0;
          const rTime = wkData.runs?.[d]?.time || '0';
          const rParts = rTime.split(':').map(Number);
          const rMin = rParts.length === 2 ? rParts[0] + rParts[1]/60 : parseInt(rTime, 10) || 0;
          if (rRpe > 0 && rMin > 0) runLoad += rRpe * rMin;
        });
      }
      lift.push(gymLoad);
    ...
    const acute = (lift[currentWkNum - 1] || 0) + (run[currentWkNum - 1] || 0);
    const validWeeks = lift.slice(0, currentWkNum).map((v, idx) => v + run[idx]).filter(v => v > 0);
    
    if (acute > 0 && validWeeks.length > 0) {
      const chronic = validWeeks.reduce((a, b) => a + b, 0) / validWeeks.length;
      ...
      if (acwr >= 1.4) {
        insights.push({
          ...
          text: `Your calculated Acute:Chronic Workload Ratio is sitting high at ${acwr.toFixed(2)}. You have spiked accumulated fatigue relative to your historical tissue baseline, placing you inside the injury danger zone. Consider cutting supplemental working sets.`
        });
      }
    }
  } catch (e) { console.warn("ACWR insight engine error:", e); }
